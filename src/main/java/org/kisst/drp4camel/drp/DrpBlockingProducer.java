 ...
 */
public class DrpBlockingProducer extends DefaultAsyncProducer {
    private static final Logger LOG = LoggerFactory.getLogger(DrpBlockingProducer.class);
    private final DrpEndpoint endpoint;

    public DrpBlockingProducer(DrpEndpoint endpoint) {
        super(endpoint);
    ...
    protected DrpConsumer getConsumer(Exchange exchange) throws Exception {
        DrpConsumer answer = endpoint.getConsumer();
        if (answer == null) {
            // okay then await until we have a consumer or we timed out
            answer = awaitConsumer();
            if (answer == null) {
                throw new DrpConsumerNotAvailableException("No consumers available on endpoint: " + endpoint, exchange);
            }
        }
    ...
    private DrpConsumer awaitConsumer() throws InterruptedException {
        DrpConsumer answer = null;
